#' Simulate Dose-Response Analysis Across Study Design Scenarios
#'
...
#'
#' @param Mods A single `Mods` object or a named list of `Mods` objects with different max effect levels.
...
#' @param mod_weights Vector of model probabilities for the models specified in ‘⁠models⁠’, assumed in the same order as specified in Mods
...
#' MaxEff_vals <- c(3, 4)
#' Mods <- setNames(lapply(MaxEff_vals, function(maxEff) {
#'   DoseFinding::Mods(
#'     emax = 80,
#'     sigEmax = c(50, 3.5),
#'     quadratic = -0.004,
...
#'
#' results <- simulate_dose_response_analysis(
#'   Mods = Mods,
#'   sigma = 0.3,
#'   doses = doses,
#'   delta_grid = c(0.5, 1, 1.5),
#'   mod_weights = c(emax = 0.5, quadratic = 0.25, sigEmax = 0.25),
#'   go_threshold = 1.5,
#'   separation_threshold = 0.3,
#'   nSim = 100
#' )
#' results$MaxEff_3$target_dose_summary$table
simulate_dose_response_analysis <- function(
    Mods,
    sigma,
    doses,
    delta_grid,
  ...
  # Get all model names used across all MaxEff levels in Mods - replaced with code below
  # all_models_used <- unique(unlist(
  #   lapply(Mods, function(mod_group) {
  #     if (is.list(mod_group)) names(mod_group) else NULL
  #   })
  # ))

  # Get all model names used across all MaxEff levels in Mods
  get_model_names <- function(mods_obj) {
    model_list <- names(mods_obj)
    unlist(lapply(model_list, function(m) {
      n_rows <- nrow(mods_obj[[m]])
      if (is.null(n_rows)) {
        return(m)
      } else if (n_rows == 1) {
        return(m)
      } else {
        return(paste0(m, seq_len(n_rows)))
      }
    }))
  }

  all_models_used <- get_model_names(pMods[[1]])

  # If mod_weights is NULL, assign equal weights
  if (is.null(mod_weights)) {
    mod_weights <- setNames(rep(1 / length(all_models_used), length(all_models_used)),
                            all_models_used)
  }

  # Check for at least one MaxEff level with multiple models
  has_multiple_models <- any(sapply(Mods, function(mod_group) {
    is.list(mod_group) && length(mod_group) > 1
  }))

  # Check that model names match those in weights
  if (!setequal(names(mod_weights), all_models_used)) {
    stop("Mismatch between model names in `Mods` and `mod_weights`.\n",
         "Models in Mods: ", paste(sort(all_models_used), collapse = ", "), "\n",
         "Models in mod_weights: ", paste(sort(names(mod_weights)), collapse = ", "))
  }

  # Check that the number of models matches the number of weights
  if (length(all_models_used) != length(mod_weights)) {
    stop("The number of models in `Mods` does not match the number of weights specified.")
  }

  # Check that weights sum to 1 (within a small numerical tolerance)
  if (!isTRUE(all.equal(sum(mod_weights), 1, tolerance = 1e-8))) {
    stop("The model weights must sum to 1. Current sum: ", sum(mod_weights))
  }

# -------------------------------------------------------------------------------------------
  results <- list()

  if (inherits(Mods, "Mods")) {
    Mods <- list(SingleScenario = Mods)
  }
  ...
  for (MaxEff_name in names(Mods)) {
    # MaxEff_name = "MaxEff_3"
    mods <- Mods[[MaxEff_name]]
    ...
    if(has_multiple_models) {
      ...
      # Identify columns to average (excluding group and Delta)
      cols_to_avg <- summary_data |>
        dplyr::select(where(is.numeric), -Delta) |>
        colnames()

      # Compute weighted average for each Delta
      averaged_rows <- summary_data |>
        dplyr::filter(group %in% names(mod_weights)) |>
        dplyr::group_by(Delta) |>
        dplyr::summarise(across(
          .cols = all_of(cols_to_avg),
          .fns = ~ weighted.mean(.x, w = mod_weights[group]),
          .names = "{.col}"
        ), .groups = "drop") |>
        dplyr::mutate(group = "averaged") |>
        dplyr::select(group, Delta, all_of(cols_to_avg))

      # Combine with original tibble
      summary_data <- dplyr::bind_rows(summary_data, averaged_rows)
    }
    ...
    for (true_mod in names(modelSel)) {
      ...
      for (i in seq_len(nSim)) {
        ...
        cf <- try(coefs[[true_mod]][[i]], silent = TRUE) # changed mod to true_mod which is the correct pick
    ...
    if(has_multiple_models) {
      ...
      # Add Design Averaging Module:
      all_data <- purrr::imap_dfr(dose_response_quantiles_long[names(mod_weights)],
                                  ~ dplyr::mutate(.x, model = .y))

      averaged_df <- all_data |>
        dplyr::group_by(dose, simulation, quantile) |>
        dplyr::summarise(value = weighted.mean(value, w = mod_weights[model]), .groups = "drop")

      dose_response_quantiles_long$averaged <- averaged_df
    ...
    if(has_multiple_models) {
      ...
      # Step 3: Stack into a 3D array: [row, col, model]
      array_data <- abind::abind(dose_response_profiles[names(mod_weights)], along = 3)

      # Step 4: Compute weighted average along the model dimension
      averaged_matrix <- apply(array_data, c(1, 2), function(x) weighted.mean(x, w = mod_weights))

      # Step 5: Convert to data.frame and preserve colnames
      averaged_df <- as.data.frame(averaged_matrix)
      colnames(averaged_df) <- colnames(dose_response_profiles[[1]])

      # Step 6: Add to list
      dose_response_profiles$averaged <- averaged_df
    ...
    if(has_multiple_models) {
      ...
      # Compute weighted average across rows by column
      td_averaged <- td_summary |>
        dplyr::filter(model %in% names(mod_weights)) |>
        dplyr::summarise(across(
          .cols = where(is.numeric),
          .fns = ~ weighted.mean(.x, w = mod_weights[td_summary$model]),
          .names = "{.col}"
        )) |>
        dplyr::mutate(model = "averaged") |>
        dplyr::select(model, everything())

      # Step 5: Append to original tibble
      td_summary <- dplyr::bind_rows(td_summary, td_averaged)
